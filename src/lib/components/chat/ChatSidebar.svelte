	...
	{#if !isCollapsed}
			...
			{:else}
				<div class="space-y-1 p-2">
					...
					<div
							...
							on:keydown={(e) => (e.key === 'Enter' || e.key === ' ') && selectConversation(conversation.id)}
							class="group relative p-3 rounded-lg cursor-pointer transition-all duration-200 hover:scale-[1.02] w-full text-left {state.activeConversationId === conversation.id ? 'bg-sky-50 dark:bg-sky-900/50 border-l-4 border-sky-500 shadow-md' : 'hover:bg-stone-100 dark:hover:bg-stone-700/50'}"
					>	<div class="flex items-start justify-between">
							<div class="flex-1 min-w-0">
               ...
               {conversation.metadata.messageCount}
                ...
                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-sky-100 dark:bg-sky-900 text-sky-800 dark:text-sky-200 font-medium">
                 {tag}
							...
							<div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-all duration-200" role="group" on:click|stopPropagation on:keydown|stopPropagation>
								<div class="flex space-x-1">
									<button
										...
										aria-label={conversation.isFavorite ? 'Quitar de favoritos' : 'Agregar a favoritos'}
									>
		...
		<div class="border-t border-stone-200/60 dark:border-stone-700/60 p-4 bg-stone-50/30 dark:bg-stone-900/30">
			<div class="text-xs text-stone-500 dark:text-stone-400 space-y-1">
				<div class="flex justify-between" role="status" aria-live="polite">
		...
		<div class="flex-1 overflow-y-auto py-2">
			{#each getConversationsToShow().slice(0, 8) as conversation (conversation.id)}
				<div
					...
					aria-label={`Abrir conversación ${conversation.name}`}
